...
export const AiMessages = ({messages, showChat, chat, handleLinkClick, iaResponse}: {messages: {type: string, message: string}[], showChat: boolean, chat: React.RefObject<HTMLDivElement | null>, handleLinkClick: (event: any) => void, iaResponse: any}) => {
    return (
        <div
            ref={chat}
            className={`${showChat ? "opacity-100 h-[80%] overflow-auto px-4 pt-20" : "opacity-0 h-0 overflow-hidden"} basis-full sm:basis-9/12 overflow-auto max-h-130 sm:max-h-150 rounded-3xl scrollbar-width-6`}
          >
            <AnimatePresence>
